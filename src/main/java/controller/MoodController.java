...
public class MoodController {
    ...
    public MoodController(ComboBox<model.Emotions> dropdown,
                          Label resultLabel,
                          VBox historyContent,
                          Label greetingLabel,
                          VBox root) {
    ...
    public void handleSubmit() {
        model.Emotions emotion = dropdown.getValue();
        if (emotion == null) {
            ...
            if (parts.length == 3) {
                ...
                for (model.Emotions e : model.Emotions.values()) {
                    if (moodPart.startsWith(e.getEmoji())) {
                        if (e == model.Emotions.HAPPY || e == model.Emotions.CALM || e == model.Emotions.MOTIVATED || e == model.Emotions.HOPEFUL) {
                            entryLabel.setTextFill(Color.LIGHTGREEN);
                        } else if (e == model.Emotions.INDIFFERENT) {
    ...
    private Color getColor(model.Emotions emotion) {
